...
export default function NpmSection() {
  ...
  const stats = [
    { icon: Download, label: "Total Downloads", value: "1,500+" },
    { icon: Zap, label: "Events / Sec", value: "1k" },
    { icon: Activity, label: "Avg log Latency", value: "<300ms" },
  ];
  ...
  return (
    ...
    >
      {" "}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-blue-500/10 rounded blur-[120px] pointer-events-none" />
      <div className="text-center mb-16 mx-auto max-w-4xl relative z-10">
        ...
        >
          ...
          <span className="absolute bottom-0 left-1/2 -translate-x-1/2 h-1 w-1/3 bg-purple-500 rounded transition-all duration-500 group-hover:w-full opacity-50 group-hover:opacity-100" />
        </motion.h2>
          ...
          <div className="px-5 py-4 flex items-center justify-between gap-3 flex-wrap border-b border-slate-200 dark:border-white/[0.05]">
            ...
            <div className="flex gap-2">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded bg-slate-100 dark:bg-white/5 border border-transparent dark:border-white/5 hover:bg-slate-200 dark:hover:bg-white/10 transition-all text-slate-700 dark:text-slate-300"
              >
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded bg-blue-50 dark:bg-blue-500/10 border border-blue-100 dark:border-blue-500/20 hover:bg-blue-100 dark:hover:bg-blue-500/20 transition-all text-blue-700 dark:text-blue-400"
              >
          ...
          <div className="p-5 flex flex-col gap-6">
            ...
            <div className="grid grid-cols-3 gap-3">
              {stats.map(({ icon: Icon, label, value }) => (
                <div
                  key={label}
                  className="flex flex-col items-center justify-center gap-1.5 bg-slate-50 dark:bg-black/40 border border-slate-100 dark:border-white/5 rounded py-3 px-2"
                >
                  <Icon
                    size={16}
            ...
            <div className="flex flex-wrap gap-2">
              {tags.map((tag) => (
                <span
                  key={tag}
                  className="text-xs px-2.5 py-1 rounded bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/5 text-slate-600 dark:text-slate-300"
                >
                  {tag}
                </span>
              ))}
            ...
            <div className="group flex items-center justify-between bg-slate-900 dark:bg-black/60 border border-slate-800 dark:border-white/10 rounded p-1 pl-4 mt-2 hover:border-slate-700 dark:hover:border-white/20 transition-colors">
              ...
              <button
                onClick={copy}
                className="flex items-center justify-center w-10 h-10 rounded bg-slate-800 dark:bg-white/5 hover:bg-slate-700 dark:hover:bg-white/10 transition-colors flex-shrink-0"
